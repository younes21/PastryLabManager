...
const InventoryPhysicalInterface = () => {
  ...
  useEffect(() => {
    if (isEditing && selectedZoneId) {
      setCurrentOperation((prev: any) =>
        prev ? { ...prev, storageZoneId: selectedZoneId } : prev,
      );
    ...
    try {
      ...
      setItems(
        (data.items || []).map((it: any) => ({
          ...
          article: articles.find((a: any) => a.id === it.articleId) || {
            id: it.articleId,
          },
  ...
  const cancelOperation = async (op: any) => {
    if (
      !confirm(
        "Êtes-vous sûr de vouloir annuler cet inventaire ? Cette action est irréversible.",
      )
    )
      return;
  ...
  const addItem = (article: any) => {
    ...
    const existingItem = items.find(
      (item) =>
        item.articleId === article.id && item.storageZoneId === selectedZoneId,
    );
    if (!existingItem) {
      const newItem = {
  ...
  const updateItemUnitCost = (itemId: number, newUnitCost: string) => {
    setItems(
      items.map((item) =>
        item.id === itemId
          ? { ...item, unitCost: parseFloat(newUnitCost) || 0 }
          : item,
      ),
    );
  };
                ...
                .map((op) => {
                  ...
                  return (
                    <TableRow key={op.id} className={rowClass}>
                      ...
                      <TableCell>
                        {storageZones.find((z) => z.id === op.storageZoneId)
                          ?.designation || "-"}
                      </TableCell>
  ...
  usePageTitle(
    "Inventaire Physique > " + (isViewing ? "Consultation" : "Édition"),
  );
  return (
    <div>
      {/* Header */}
                    ...
                    <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg">
                      ...
                      <div className="max-h-80 overflow-y-auto">
                        {filteredArticles.map((article) => {
                          ...
                          const articleZone = storageZones.find(
                            (z) => z.id === article.storageZoneId,
                          );
                          const displayZone =
                            articleZone?.designation || "Zone par défaut";
            ...
            <Table>
              ...
              <TableBody>
                {items
                  ...
                  .map((item, index) => {
                    const zone = storageZones.find(
                      (z) => z.id === item.storageZoneId,
                    );
                    ...
                    return (
                      <TableRow key={item.id}>
                        ...
                        <TableCell className="text-center p-2">
                          {isViewing ? (
                            <span
                              className={`text-sm font-medium ${item.newQuantity === 0 ? "text-red-600" : ""}`}
                            >
                          ...
                          <TableCell className="text-center p-2">
                            {isViewing ? (
                              ...
                              <Input
                                ...
                                value={item.unitCost ?? ""}
                                onChange={(e) =>
                                  updateItemUnitCost(item.id, e.target.value)
                                }
                                min="0"
                                step="0.01"
